...
one row per currently completed order with customer context.
...
- `RISK_REPEAT_HIGH_VALUE_ORDERS`: alerts when a customer has at least the
  configured number of currently completed orders inside the regional rolling
  window, with every order at or above the regional per-order amount. The alert
  records the qualifying count and total amount.

The repeat-order window is exclusive at its lower boundary and inclusive at
the supplied monitoring time. It uses `ordered_at` because this compact model
does not store a separate completion timestamp. Orders currently marked
`PENDING` or `CANCELLED` are excluded.

All rules use regional configuration. A partial unique index prevents more
